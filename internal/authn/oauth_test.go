...
func TestSupportedScopesIncludeLegacyAndGranularWrites(t *testing.T) {
	...
	for _, expected := range []string{readScope, writeScope, ServiceWriteScope("rental", "create"), ServiceWriteScope("warehouse", "update"), ServiceWriteScope("procurement", "update"), ServiceWriteScope("procurement", "approve"), ServiceWriteScope("procurement", "submit"), ServiceWriteScope("procurement", "receive")} {
