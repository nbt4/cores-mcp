...
func TestWriteToolsExposeSafeAnnotationsAndSchemas(t *testing.T) {
	...
	if len(tools) != 114 {
		t.Fatalf("tool count = %d, want 114", len(tools))
	}
	...
	for _, name := range []string{"procurement.products.create", "procurement.offers.create", "procurement.requisitions.create", "procurement.suppliers.create", "procurement.categories.create", "rental.jobs.create", "rental.requirements.create", "planner.plans.create", "planner.tasks.create", "warehouse.tasks.create", "warehouse.products.create", "rental.jobs.assign_device", "procurement.orders.create"} {
		tool := tools[name]
	...
	for _, name := range []string{"rental.jobs.update", "rental.requirements.update", "warehouse.movements.create", "warehouse.devices.update_status", "warehouse.products.update", "procurement.products.update", "procurement.offers.update", "procurement.requisitions.update", "procurement.requisitions.submit", "procurement.suppliers.update", "procurement.categories.update", "procurement.requisitions.decide", "procurement.orders.receive"} {
		tool := tools[name]
		if tool == nil || tool.Annotations == nil || tool.Annotations.ReadOnlyHint || !tool.Annotations.IdempotentHint || tool.Annotations.DestructiveHint == nil || !*tool.Annotations.DestructiveHint {
