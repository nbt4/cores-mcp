...
func TestWriteToolsExposeSafeAnnotationsAndSchemas(t *testing.T) {
	...
	if len(tools) != 96 {
		t.Fatalf("tool count = %d, want 96", len(tools))
	}
	...
	for _, name := range []string{"rental.jobs.update", "rental.requirements.update", "warehouse.movements.create", "warehouse.devices.update_status", "procurement.suppliers.update", "procurement.requisitions.decide", "procurement.orders.receive"} {
		tool := tools[name]
		if tool == nil || tool.Annotations == nil || tool.Annotations.ReadOnlyHint || !tool.Annotations.IdempotentHint || tool.Annotations.DestructiveHint == nil || !*tool.Annotations.DestructiveHint {
